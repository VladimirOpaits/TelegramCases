...
@app.post("/payment/retry_verification")
async def retry_payment_verification(
    request: dict,
    current_user_id: int = Depends(get_current_user_id)
):
    """Повторная проверка транзакции (особенно полезно для тестнета)"""
    payment_id = request.get("payment_id")
    transaction_hash = request.get("transaction_hash")
    ...
    # Проверяем, что платеж в статусе failed (для повторной проверки)
    if payment.status != 'failed':
        raise HTTPException(
            status_code=400, 
            detail=f"Платеж в статусе {payment.status}. Повторная проверка доступна только для неудачных платежей."
        )
    
    # Повторно проверяем транзакцию
    verification = await payment_manager.verify_ton_transaction(
        transaction_hash=transaction_hash,
    ...
    if verification.is_valid:
        # Если транзакция теперь валидна, добавляем фантики
        success, message, new_balance = await db_manager.atomic_add_fantics(
            payment.user_id, 
            payment.amount_fantics
        )
        
        if success:
            # Помечаем платеж как подтвержденный
            await db_manager.update_payment_status(payment_id, 'confirmed', transaction_hash)
            
            return {
                "success": True,
                "message": f"Транзакция подтверждена! Добавлено {payment.amount_fantics} фантиков",
                "new_balance": new_balance,
        ...
        return {
            "success": False,
            "message": f"Транзакция все еще не подтверждена: {verification.message}",
            "verification_details": {
                "amount_sent": verification.amount_sent,
                "sender_address": verification.sender_address,
                "block_number": verification.block_number
